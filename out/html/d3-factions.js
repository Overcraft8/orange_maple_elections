...
d3.factions = function() {
    ...
    /* events */
    var factionsDispatch = d3.dispatch("click", "dblclick", "mousedown", "mouseenter",
        "mouseleave", "mousemove", "mouseout", "mouseover", "mouseup", "touchcancel", "touchend",
        "touchmove", "touchstart");

    function factions(data) {
        console.log(data);
        data.each(function(d) {
            ...
            var outerfactionsRadius = Math.min(width/2, height);
            var innerParliementRadius = outerfactionsRadius * innerRadiusCoef;

            /* init the svg */
            var svg = d3.select(this);

            /***
             * compute number of seats and rows of the factions */
            var nSeats = 0;
            ...
            var rowWidth = (outerfactionsRadius - innerParliementRadius) / nRows;
            var seats = [];
            (function() {
            ...
            /* container of the factions */
            var container = svg.select(".factions");
            if (container.empty()) {
                container = svg.append("g");
                container.classed("factions", true);
            }
            container.attr("transform", "translate(" + width / 2 + "," + outerfactionsRadius + ")");

            /* all the seats as circles */
            var circles = container.selectAll(".seat").data(seats);
            circles.attr("class", seatClasses);

            /* animation adding seats to the factions */
            ...
            for (var evt in factionsDispatch._) {
                (function(evt){
                    circlesEnter.on(evt, function(e) { factionsDispatch.call(evt, this, e); });
                })(evt);
            }

            /* animation updating seats in the factions */
            if (update.animate) {
            ...
            /* animation removing seats from the factions */
    ...
    factions.height = function(value) {
        if (!arguments.length) return height;
        return factions;
    };

    factions.innerRadiusCoef = function(value) {
    ...
    factions.on = function(type, callback) {
        factionsDispatch.on(type, callback);
    }
